    ...
    Vue.component('table-view', {
        ...
        methods: {
            initializeQuickFilter: function() {
                if (this.body_area.attributes?.filterConfig?.quick) {
                    this.quickfilter = {...this.body_area.attributes.filterConfig.quick};
                }
            },
            initializeArrfil: function() {
            ...
            generateSecureId(length) {
                ...
                for (const value of values) {
                    secureId += charset[value % charset.length];
                }
            
                return secureId;
            },
            createItemContent: function() {
                this.item_content = '<table id=rtaTable-' + this.id + ' class="display nowrap table table-striped table-bordered table-sm" cellspacing="0" width="100%"></table>';
            },
            handleTypeFilter2: function() {
                if (this.body_area?.attributes?.filterConfig?.typeFilter == 2) {
                    let json_filter = {
                        column: '__typeFilter2__',
                        entries: this.body_area?.attributes?.filters ?? [],
                        screen_code: this.body_area.screenCode,
                        title: vm.lang === 'vi' ? 'Từ khóa thông dụng' : 'Most popular keyworks',
                        type: 'typeFilter2',
                        table: 'table'
                    };
                    this.body_area.attributes.filterConfig.typeFilter = 1;
                    vm.activeListFilters.push(json_filter);
                }
            },
            handleTaskCode9999: function() {
                if (this.task.code === 9999) {
                    this.processScreens();
                }
            },
            processScreens: function() {
                let object = this.object;
                for (let screen_code in object.screens) {
                    if (object.screens.hasOwnProperty(screen_code) && screen_code == this.body_area.screenCode) {
                        this.processBodyArea(object.screens[screen_code]['body_area']);
                    }
                }
            },
            processBodyArea: function(body_area) {
                for (let body_code in body_area) {
                    if (body_area.hasOwnProperty(body_code)) {
                        this.processTableView(body_area[body_code]);
                    }
                }
            },
            processTableView: function(body_area_item) {
                if (body_area_item.type == 'tableView' && body_area_item.attributes.hasOwnProperty('filters')) {
                    this.processFilters(body_area_item.attributes.filters, body_area_item.code);
                }
            },
            processFilters: function(filters, table_code) {
                filters.forEach(filter => {
                    this.prepareFilter(filter, table_code);
                    this.addFilterToActiveList(filter);
                });
            },
            prepareFilter: function(filter, table_code) {
                filter.code = 'filter-' + makeElemId();
                filter.screen_code = this.body_area.screenCode;
                if (!filter.hasOwnProperty('entries')) {
                    filter.entries = [];
                }
                filter.table = table_code;
            },
            addFilterToActiveList: function(filter) {
                let check = true;
                if (vm.activeListFilters.length > 0) {
                    vm.activeListFilters.forEach(ft => {
            ...
            handleFilter2(keyword){
                let json = this.dataStore
                let data
                if(keyword[0] === '_all'){
                    data = this.dataStore
                }else{
                    data = json.filter(obj => {
                        return Object.keys(obj).some(key => {
                            let pattern = /^([^(\s]+)/;
                            keyword = keyword.map(function(key) {
                                let match = pattern.exec(key);
                                return match[1];
                            });
                            return keyword.some(value => String(obj[key]).includes(value))
                        });
                    });
                }
                this.renderFilterTable(data)
            },
            filterTableview:function(item_filter_attributes,check=false){

                let filter_parse = JSON.parse(item_filter_attributes)
                if(filter_parse.hasOwnProperty('__typeFilter2__')){
                    this.handleFilter2(filter_parse['__typeFilter2__'])
                    return
                }
            ...
            getDataTable: function(ctv,paging,dataSet,columns,columnDefs,order) {
                ...
                setTimeout(() => {
                     $("#rtaTable-"+that.id+"_wrapper .dm-table-view").DataTable().columns.adjust();
                }, 10);
                let page = 0;
