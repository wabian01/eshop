<template id='floating_button'>
    <div v-if="list_items && list_items.length > 0" class='floating_button'>
        <div v-if="list_items.length <= 2">
            <action-button v-for="(action, index) in list_items" :indexsearch="true" :end="list_items.length-1" :key="action" :item_button="action" :task="task" :list_item="list_items" :floating_button="true" style="margin-right:10px;" :appearance="screen.floating_buttons.appearance"> </action-button>
        </div>
