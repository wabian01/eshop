var codeString = "(" + function() {
        var dmobj = [];
        var vm = {}
        function handleConditionRuleQuotes(condition,item,flatRuntimeAttributes,parent){
            let regex = /"([^"]*)"|'([^']*)'/g;
            return condition = condition.replace(regex,function(data){
                data = data.replace(/##(.*?)##/g, function(match, capture){
                ...
                if(data.indexOf('##')>-1){
                    for (var key in flatRuntimeAttributes) {
                        if (flatRuntimeAttributes.hasOwnProperty(key)) {
                            data = data.replace(new RegExp('##'+key+'##','g'),flatRuntimeAttributes[key].replace(/[\r\n]+/g," "));
                        }
                    }
                }
                ...
                for(var key in parent){
                    try {
                        data = data.replace(new RegExp('##'+key+'##','g'),parent[key].replace(/[\r\n]+/g," "));
                    } catch (error) {
                    }
                }
        ...
        function handleConditionRuleNotQuotes(condition,item,flatRuntimeAttributes,parent){
            ...
            if(condition.indexOf('##')>-1){
                for (var key in flatRuntimeAttributes) {
                    if (flatRuntimeAttributes.hasOwnProperty(key)) {
                        condition = condition.replace(new RegExp('"##'+key+'##"','g'),"\""+flatRuntimeAttributes[key].toString().replace(/[\r\n]+/g," ")+"\"");
                        condition = condition.replace(new RegExp('##'+key+'##','g'),"'"+flatRuntimeAttributes[key].toString().replace(/[\r\n]+/g," ")+"'");
                    }
                }
            }
            ...
            for(var key in parent){
                try {
                    condition = condition.replace(new RegExp('##'+key+'##','g'),'"'+parent[key].replace(/[\r\n]+/g," ")+'"');
                } catch (error) {
                }
            }
            if (condition.indexOf('##') > -1) {
            ...
            return data.map(item => {
                rules.map(rule => {
                    if (rule.hasOwnProperty('if')) {
                        let check = rule.if
                        if (check.indexOf('this()') > -1) {
                            check = check.replace(/this\(\)/g, '##' + rule.key + '##')
                        }
                        
                        check = handleConditionRuleQuotes(check,item,flatRuntimeAttributes, parent)
                        check = handleConditionRuleNotQuotes(check,item,flatRuntimeAttributes, parent)
                        
                        try {
                            let checked = eval(check);
                            if (typeof(checked) === 'boolean' && checked) {
                                if (rule.function.indexOf('##') > -1) {
                                    let checkfunc = rule.function
                                    if (checkfunc.indexOf('this()') > -1) {
                                        checkfunc = checkfunc.replace(/this\(\)/g, '##' + rule.key + '##')
                                    }
                                    
                                    checkfunc = handleConditionRuleQuotes(checkfunc,item,flatRuntimeAttributes, parent)
                                    checkfunc = handleConditionRuleNotQuotes(checkfunc,item,flatRuntimeAttributes, parent)
                        ...
                        func = handleConditionRuleQuotes(func,item,flatRuntimeAttributes, parent)
                        func = handleConditionRuleNotQuotes(func,item,flatRuntimeAttributes, parent)
