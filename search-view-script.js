        ...
        methods: {
            ...
            dynamicFilterQuick(data){
                try {
                    ...
                    if(data.hasOwnProperty('request')){
                        ...
                        let dataPparam =  (data.request.method != undefined && data.request.method.toLowerCase() == 'get') ? JSON.parse(data.request.body) : data.request.body
            ...
            searchData:function(valuesearch,scroll="?size=50"){
                ...
                this.getStatus = $.ajax({
                    ...
                    headers: (that.object.dm_type === "Custom" || that.object.dm_type == "Elasticsearch") ? {
                    ...
                    success:function (data) {
                        ...
                        if(data.length>0){
                            ...
                            that.dataLazy = that.dataLazy.concat(structuredClone(data))
                            that.lazyLoading()
                            that.notfound=""
                            if(!status_scroll){
                                that.outData = true
            ...
            scrollLazy(event){
                if(($(event.target).prop('scrollHeight')-($(event.target).scrollTop()+$(event.target).prop('clientHeight'))<=($(event.target).prop('scrollHeight')/3)) && !this.scrollEnd && !this.isLoading){
                    this.list_items = this.dataLazy.slice(0,10*this.currentPage);
                    if(this.list_items.length === this.dataLazy.length){
                        if(this.outData){
                            this.scrollEnd = true
                            return
                        }
            ...
            loadMoreData() {
                this.list_items = this.dataLazy;
                this.currentPage = Math.ceil(this.dataLazy.length / 10);
            },
            ...
            searchOffline(data){
                this.outData = true
                this.dataLazy = structuredClone(data)
                this.lazyLoading()
            },
            deleteItem(value){
